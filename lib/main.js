...
function updateSectionTeacher(value) {
    ...
    switch (value.status) {
        case "created":
            ...
            break;
        case "started":
            if (currentResponses && currentResponses.responses) createButton(controls, "Show Cards to Students", studentsResponded);
            ...
            document.getElementById(BTN_EXPORT_ID).hidden = !objectHasAnySubKey(value, "responses");
            if (currentResponses && currentResponses.responses) {
                ...
                const h = createHeading(teacherDiv, `Current sentence:`);
                h.className = "mx-3 text-center";
                h.hidden = !showCards;
                const sentence = createHeading(teacherDiv, `${currentResponses.sentence}`);
                sentence.className = "mx-3 text-center display-3";
                sentence.hidden = !showCards;
            }
...
function createExportCloseButtons(parent) {
    const btnExport = createButton(parent, "Export");
    btnExport.setAttribute("data-bs-toggle", "modal");
    btnExport.setAttribute("data-bs-target", `#${MODAL_DOWNLOAD_ID}`);
    btnExport.id = BTN_EXPORT_ID;
    createButton(parent, "Close Session", closeCurrentRoom);
}

function createSectionTeacherForm() {
    contentTag.innerHTML = "";
    const div = createSmallCard(contentTag, "Create New Session");

    const inpNewSessionNumber = createInputSet(div, INP_NEW_SESSION_NUMBER_ID, "number", "Enter your session number:");
    const inpNumberDiv = inpNewSessionNumber.parentElement;
    newRoomNumber(false);
    const row = createRow(document.body);
    const inputCol = createCol(row);
    inpNewSessionNumber.before(row);
    inputCol.appendChild(inpNewSessionNumber);
    const rollCol = createCol(row);
    rollCol.classList.replace("col", "col-auto");
    rollCol.classList.replace("my-auto", "mt-auto");
    createButton(rollCol, ICON_RANDOM, newRoomNumber);
    inpNumberDiv.classList.remove("mb-3");
    inpNumberDiv.classList.add("mb-0");
    const pNumber = createParagraph(inpNumberDiv, `i{Use a number with at least 5 digits as your code.}`);
...
function newRoomNumber(isRandom = true) {
    const inpNewSessionNumber = document.getElementById(INP_NEW_SESSION_NUMBER_ID);
    const storageRoomId = localStorage.getItem(ROOM_KEY);
    if (storageRoomId && !isRandom) {
        inpNewSessionNumber.value = storageRoomId;
        return;
    }
    inpNewSessionNumber.readOnly = true;
    getRandomRoomNumber().then(roomId => {
        inpNewSessionNumber.value = roomId;
        inpNewSessionNumber.readOnly = false;
    });
}
