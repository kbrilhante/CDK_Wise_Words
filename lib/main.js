...
function updateSectionStudent(value) {
    ...
    switch (value.status) {
        case 'created':
            ...
            break;
        case "started":
            hasResponded().then(studentHasResponded => {
    ...
    switch (value.status) {
        ...
        case "started":
            ...
            if (currentResponses.responses) {
                ...
                h.hidden = !showCards;
                const cardsRow = createRow(teacherDiv);
                ...
                fillCardsResponses(cardsRow, currentResponses);
            }
            ...
            break;
        case "responded":
            createButton(controls, "New round");
            createExportCloseButtons(controls);
            break;
    }
}
 ...
 * 
 */
function fillCardsResponses(parent, value) {
    for (const resp of value.responses) {
        ...
        if (!studentName) {
            btn = createButton(
                createButtonSection(card), 
                ICON_TRASH_CAN,
                () => {removeStudentResponse(value.responses.indexOf(resp))}
            );    
        } else {
            // TODO like button
            btn = createButton(
        ...
        btn.parentElement.classList.replace('mx-auto', 'ms-auto');
...
function createExportCloseButtons(parent) {
    ...
    // TODO remove this after configuring the download feature
    console.log("remove this after modal finished");
    btn.click();
}
