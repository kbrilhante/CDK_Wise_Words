    ...
    if (fields) {
        checkRoom(fields.sessionNumber).then(roomExists => {
            ...
            if (!errorMessage(NEW_SESSION_ERROR_MESSAGE_ID, txtError, roomExists)) {
                const input = document.getElementById(INP_NEW_SESSION_PASS_ID);
                input.hidden = true;
                input.type = "text";
...
function newSessionRound() {
    if (confirm("Start a new round?")) {
        renewSession(currentRoom);
        getStudents(currentRoom).then(students => {
...
function startActivity() {
    setStatus(currentRoom, "started");
    getNewSentence(currentRoom).then(sentence => {
        setNewSentence(currentRoom, sentence);
    });
}

function studentsResponded() {
    setStatus(currentRoom, "responded");
}

function showCurrentSentence() {
    getCurrentSentence(currentRoom).then(sentence => {
        const sentenceHeader = document.getElementById(STUDENT_SENTENCE_ID);
        sentenceHeader.innerText = sentence;
        if (sentence) canReload = false;
    });
}
...
function sendStudentResponse() {
    ...
    if (response) {
        canReload = true;
        addStudentResponse(currentRoom, response);
    }
}

function checkStudentDeleted() {
    studentExists(currentRoom, studentName).then(exists => {
        if (!exists) kickStudent("Student was removed from the room. Please login again.");
    });
}

function checkRoomClosed() {
    checkRoom(currentRoom).then(exists => {
        if (!exists) kickStudent("Room was closed. Thanks for playing.")
    })
}
...
function closeCurrentRoom() {
    if (confirm("Are you sure you want to close session? All the session info will be lost.")) {
...
function processTextFile(data) {
    ...
    for (const session of data) {
        if (!objectHasKey(session, "responses")) continue;
...
async function getRandomRoomNumber() {
    const newRoom = randInt(10000, 99999);
    const exists = await checkRoom(newRoom);
    if (!exists) return newRoom;
