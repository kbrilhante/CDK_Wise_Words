...
async function createRoomEvent(roomId, callback) {
  ...
  roomEvent = await ref.on("value", snapshot => {
    ...
    } else if (studentsListEvent) {
      studentsListEvent.off();
    }
    callback(val);
  });
}
...
async function createStudentsEvent(roomId) {
  ...
  studentsListEvent = await ref.on("value", snapshot => {
    if (snapshot.exists()) fillStudentsList(snapshot.val());
  });
}
...
async function getCurrentSentence() {
  const ref = db.ref(`rooms/${currentRoom}/content`);
  let key = await ref.child("currentSession").get();
  key = key.val();
  const sentence = await ref.child(key).child("sentence").get();
  return sentence.val();
}