...
async function studentJoin(roomId) {
  const ref = db.ref(`rooms/${roomId}`);
  const students = await ref.child("students").get();
  const update = {};
  if (!students.exists()) {
    update['/students'] = [studentName];
  } else {
    let names = students.val();
    if (!names.includes(studentName)) {
      names.push(studentName);
    }
    update['/students'] = names;
  }
  ref.update(update);
}
...
async function createRoomEvent(roomId, callback) {
  ...
  roomEvent = await ref.on("value", snapshot => {
...
async function createStudentsEvent(roomId) {
  const ref = db.ref(`rooms/${roomId}/students`);
  studentsListEvent = await ref.on("value", snapshot => {
    if (snapshot.exists()) {
      if (!studentName) {
...
async function addStudentResponse(sentence) {
  const exists = await studentExists(currentRoom, studentName)
  if (!exists) {
    alert("Student not found. Please login again").then(() => console.log("aaaa"))
    return;
  }
  if (exists) {
    const key = getCurrentSession();
    const ref = db.ref(`rooms/${currentRoom}/content/${key}`);
    const response = {}
  }
}