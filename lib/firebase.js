...
async function createRoom(roomId, adminPass) {
  ...
  const room = {
    adminPass: adminPass,
    created: created,
    expiresAt: created + 24*60*60*1000,
    content: {
      status: "created",
      currentSession: "none",
    },
...
async function archiveRoom(roomId) {
  ...
  room = room.val();
  delete room.adminPass;
  delete room.sentences;
  const archiveRef = db.ref(`archive/${roomId}_${room.created}`);
  archiveRef.set(room);
  roomRef.remove();
}
...
async function archiveExpiredRooms() {
  ...
  const exists = await checkRoom(storageRoomId);
  if (!exists) localStorage.removeItem(ROOM_KEY);
}